...
def test_database_and_repositories():
    ...
    with get_session() as session:
        now = now_utc()
        # insert
        candle = upsert_candle(
            session, "SOL", now, open=100.0, high=105.0, low=98.0, close=102.0, volume=10000.5
        )
        session.flush()
        print(f"Upserted candle ID: {candle.id}")

        # update
        candle_updated = upsert_candle(
            session, "SOL", now, open=100.0, high=105.0, low=98.0, close=103.0, volume=10000.5
        )
        session.flush()
    ...
    if cache.client:
        ...
        print(
            "Redis client not connected. Skipping Redis tests (expected if no local server running)."
        )
